...
class BartDataset(Dataset):
    ...
    def __init__(self, tokenizer: BartTokenizer, text_df: pd.DataFrame, args: Any):
        super().__init__()
...
class BartRecommender(nn.Module):
    ...
    def __init__(self, args: Any=None):
        ...
        self.tokenizer = BartTokenizer.from_pretrained(args.tokenizer_name_or_path)
...
def main_train_test(train_df, test_df, args):
    ...
    model = BartRecommender(args)
    model.to(args.device)
    if args.save_model_path != "":
        model.load(args.save_model_path)

    train_dataset = BartDataset(model.tokenizer, train_df, args)
    train_dataloader = DataLoader(
        train_dataset, 
        batch_size=args.batch_size, shuffle=True, collate_fn=collate_fn
    )
    test_dataset = BartDataset(model.tokenizer, test_df, args)
